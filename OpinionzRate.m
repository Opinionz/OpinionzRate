...
- (void)setupWithAppStoreId:(NSUInteger)appStoreID {
    ...
    if (appStoreID <= 0) /* TODO: neeed more checks. A.P. */ {
        
        [NSException raise:@"Invalid app store id" format:@"API key of %lu is invalid, it can not be 'nil' or empty", (unsigned long)appStoreID];
    }
    else {
...
- (void)promptForRating {
    ...
    if (!kRatedCurrentVersion) {
        ...
        NSLog(@"OPININONZ NOTE: Already rate this version");
    }
...
- (void)alertView:(OpinionzAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    //TODO: chage button indexes or add enum A.M.
    switch (buttonIndex) {
        case 0:
            [self rejectRate];
            break;
...
- (OpinionzAlertView *)alertView {
    ...
    //TODO: reusing OpinionzAlertView not working
//    if (!_alertView) {
    
        NSString *title = self.title;
        NSString *message = self.message;
        _alertView = [[OpinionzAlertView alloc] initWithTitle:title
                                                      message:message
                                                     delegate:self
...
- (void)rateApp {
    ...
    if (NSStringFromClass([SKStoreProductViewController class]) != nil) {
        ...
        [[self getRootViewController] presentViewController:storeViewController animated:YES completion:^{
